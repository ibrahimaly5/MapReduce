...
typedef struct {
    ...
    bool addTask(ThreadPool_work_t *task);
    std::vector<ThreadPool_work_t *> jobsToDelete;
    
    //Variables for locking
    pthread_mutex_t queueLock = PTHREAD_MUTEX_INITIALIZER;
    pthread_cond_t length_nonzero = PTHREAD_COND_INITIALIZER;
} ThreadPool_work_queue_t;

typedef struct {
    // Threads vector and tasks queue
    std::vector<pthread_t> threads;        /* ID returned by pthread_create() */
